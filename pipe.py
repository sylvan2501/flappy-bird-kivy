...
class Pipe(Widget):
    ...
    pipe_top_texture = ObjectProperty(None)
    lower_pipe_tex_coords = ListProperty((0, 0, 1, 0, 1, 1, 0, 1))
    top_pipe_tex_coords = ListProperty((0, 0, 1, 0, 1, 1, 0, 1))

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.pipe_body_texture = Image(source='./files/pipe_body.png').texture
        self.pipe_body_texture.wrap = 'repeat'
        self.pipe_top_texture = Image(source='./files/pipe_cap.png').texture
        self.pipe_top_texture.wrap = 'repeat'
